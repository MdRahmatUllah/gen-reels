...
def test_alembic_upgrade_and_downgrade(tmp_path: Path, monkeypatch):
    ...
    assert "series_script_revisions" in inspector.get_table_names()
    assert "series_video_runs" in inspector.get_table_names()
    assert "series_video_run_steps" in inspector.get_table_names()
